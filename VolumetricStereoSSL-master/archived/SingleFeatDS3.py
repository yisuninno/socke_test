...
class GetCostVolume(nn.Module):
    ...
    def forward(self, x, y):
        ...
        with torch.cuda.device_of(x):
            ...
            cost = x.new().resize_(num, 6, self.maxdisp, height, width).zero_()
...
class Model(nn.Module):
    def __init__(self, maxdisp=192, c=32, training=True):
        ...
        self.conv_start = BasicConv(6, c, is_3d=True, kernel_size=5, stride=3, padding=2)
    ...
    def forward(self, x, y):
        ...
        # cv = F.interpolate(cv, [self.maxdisp, cv.size()[3] * 3, cv.size()[4] * 3], mode='trilinear', align_corners=True)
