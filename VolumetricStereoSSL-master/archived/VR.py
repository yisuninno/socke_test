    ...
    def forward(self, x, y):
        ...
        with torch.cuda.device_of(x):
            ...
            for i in range(self.maxdisp):
                if i > 0 :
                    cost[:, :x.size()[1], self.maxdisp-1-i, :,i:]   = x[:,:,:,i:]
                    cost[:, x.size()[1]:, self.maxdisp-1-i, :,i:]   = y[:,:,:,:-i]
                else:
                    cost[:, :x.size()[1], self.maxdisp-1-i, :,:]   = x
...
class VolumeRender(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.cuda.device_of(x):
            ...
            block = 1.0 - sigma
            cumprod = torch.cumprod(block, dim=1)
            x2 = x
            x2[:, 1:, :, :] = cumprod[:, :self.maxdisp-1, :, :] * x[:, 1:, :, :]
            disp = Variable(torch.Tensor(np.reshape(np.array(range(self.maxdisp-1,-1,-1)), [1, self.maxdisp, 1, 1])).cuda(),
                            requires_grad=False)
            # disp = 1.0 / disp
            # disp_2 = Variable(torch.Tensor(np.reshape(np.array(range(self.maxdisp)), [2, self.maxdisp+1, 1, 1])).cuda(),
            #                 requires_grad=False)
            # disp_2 = 1.0 / disp_2
            # disp = disp - disp_2
            x2 = self.softmax(x2)
            disp = disp.repeat(x.size()[0], 1, x.size()[2], x.size()[3])
            out = torch.sum(x2 * disp, 1)
