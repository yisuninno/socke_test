...
class VolumeRender(nn.Module):
    ...
    def forward(self, x, width):
        assert (x.is_contiguous() == True)
        with torch.cuda.device_of(x):
            #num, channels, _, height, width = x.size()
            x = torch.squeeze(x, 1)
            through = torch.sigmoid(x)
            through_1 = through[:, :, :, :width]
            block_1 = 1.0 - through_1
            cumprod = torch.cumprod(block_1, dim=1)
            w = through_1.clone()
            w[:, 1:, :, :] = cumprod[:, :self.maxdisp-1, :, :] * through_1[:, 1:, :, :]
            disp = Variable(torch.Tensor(np.reshape(np.array(range(self.maxdisp-1,-1,-1)), [1, self.maxdisp, 1, 1])).cuda(),
                            requires_grad=False)
            #disp = disp.repeat(x.size()[0], 1, x.size()[2], x.size()[3])
            disp = disp.repeat(x.size()[0], 1, x.size()[2], width)
            out = torch.sum(w * disp, 1)

            through_2 = torch.zeros_like(through_1)
            for i in range(self.maxdisp):
                if i > 0:
                    through_2[:, self.maxdisp-1-i, :, :-i] = through_1[:, self.maxdisp-1-i, :, i:]
                else:
                    through_2[:, self.maxdisp - 1 - i, :, :] = through_1[:, self.maxdisp - 1 - i, :, :]
            block_2 = 1.0 -through_2
            cumprod_2 = torch.cumprod(block_2, dim=1)
            w2 = through_2.clone()
            w2[:, 1:, :, :] = cumprod_2[:, :self.maxdisp - 1, :, :] * through_2[:, 1:, :, :]
            out_2 = torch.sum(w2 * disp, 1)
...
class Model(nn.Module):
    ...
    def forward(self, x, y):
        ...
        exp_disp_3, exp_disp_3b = self.disp(cv3, width)

        if self.training:
            cv1 = self.deconv_1(cv1)
            cv2 = self.deconv_2(cv2)
            exp_disp_1, exp_disp_1b = self.disp(cv1, width)
            exp_disp_2, exp_disp_2b = self.disp(cv2, width)
            return (exp_disp_1, exp_disp_1b), (exp_disp_2, exp_disp_2b), (exp_disp_3, exp_disp_3b)
        return (exp_disp_3, exp_disp_3b)