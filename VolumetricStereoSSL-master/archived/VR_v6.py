        ...
        with torch.cuda.device_of(v):
            ...
            for i in range(self.maxdisp):
                if i > 0:
                    ...
                    c_adv_2[:, :, self.maxdisp - 1 - i, :, i:width] = y[:, :, :, :-i]
                else:
                    c_adv[:, :, self.maxdisp - 1 - i, :, :width] = x[:, :, :, :width]
                    c_adv_2[:, :, self.maxdisp - 1 - i, :, :width] = y[:, :, :, :width]
            ...
            for i in range(self.maxdisp):
                if i > 0:
                    ...
                    color_2[:, :, self.maxdisp - 1 - i, :, :-i] = c_adv[:, :, self.maxdisp - 1 - i, :, i:width]
                    # color_1[:, :, self.maxdisp - 1 - i, :, :] = color[:, :, self.maxdisp - 1 - i, :, :width]  * x + (1. - color[:, :, self.maxdisp - 1 - i, :, :width]) * y
                else:
                    block_2[:, :, self.maxdisp - 1 - i, :, :] = block[:, :, self.maxdisp - 1 - i, :, i:width+i]
                    color_2[:, :, self.maxdisp - 1 - i, :, :] = c_adv[:, :, self.maxdisp - 1 - i, :, :width]
