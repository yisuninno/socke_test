...
class VolumeRender(nn.Module):
    ...
    def forward(self, v, x, y, x_full, y_full, is_training=True):
        ...
        with torch.cuda.device_of(v):
            ...
            # # c_adv = x.new().resize_(batch_size, channels, self.maxdisp, height, width + self.maxdisp).zero_()
            # # c_adv_2 = x.new().resize_(batch_size, channels, self.maxdisp, height, width + self.maxdisp).zero_()
            # c_adv = x.new().resize_(batch_size, channels, self.maxdisp, height, width).zero_()
            # c_adv_2 = x.new().resize_(batch_size, channels, self.maxdisp, height, width).zero_()
            # for i in range(self.maxdisp):
            #     c_adv[:, :, self.maxdisp - 1 - i, :, :width] = y_full[:, :, :, self.maxdisp-i:self.maxdisp-i+width]
            #     c_adv_2[:, :, self.maxdisp - 1 - i, :, :width] = x[:, :, :, :width]
            #     # if i > 0:
            #     #     c_adv[:, :, self.maxdisp - 1 - i, :, :width] = y_full[:, :, :, self.maxdisp-i:self.maxdisp-i+width]
            #     #     # c_adv[:, :, self.maxdisp - 1 - i, :, :i] = x[:, :, :, :i]

            #     #     # c_adv[:, :, self.maxdisp - 1 - i, :, i:width] = x[:, :, :, i:width]
            #     #     c_adv_2[:, :, self.maxdisp - 1 - i, :, :width] = x[:, :, :, :width]
            #     #     # c_adv_2[:, :, self.maxdisp - 1 - i, :, width:width+i] = y[:, :, :, -i:]
            #     # else:
            #     #     c_adv[:, :, self.maxdisp - 1 - i, :, :width] = y[:, :, :, :]
            #     #     c_adv_2[:, :, self.maxdisp - 1 - i, :, :width] = x[:, :, :, :width]
            ...
            for i in range(self.maxdisp):
                if i > 0:
                    block_2[:, :, self.maxdisp - 1 - i, :, :-i] = block[:, :, self.maxdisp - 1 - i, :, i:width]
                    color_2[:, :, self.maxdisp - 1 - i, :, :-i] = c_adv_2[:, :, self.maxdisp - 1 - i, :, i:width]
                    # color_1[:, :, self.maxdisp - 1 - i, :, :] = color[:, :, self.maxdisp - 1 - i, :, :width]  * x + (1. - color[:, :, self.maxdisp - 1 - i, :, :width]) * y
                else:
                    block_2[:, :, self.maxdisp - 1 - i, :, :] = block[:, :, self.maxdisp - 1 - i, :, :width]
                    color_2[:, :, self.maxdisp - 1 - i, :, :] = c_adv_2[:, :, self.maxdisp - 1 - i, :, :width]
...
class Model(nn.Module):
    ...
    def forward(self, x, y, x_full=None, y_full=None):
        ...
        if self.training:
            ...
            color_1a, color_1b, exp_disp_1a, exp_disp_1b = self.disp(cv1, x, y, x_full, y_full)
            return (color_1a, color_1b, exp_disp_1a, exp_disp_1b), (None, None, None, None), (None, None, None, None)

        cv1 = self.deconv_1(cv1)
        exp_disp_1a = self.disp(cv1, x, y, x_full, y_full, self.training)
        return exp_disp_1a