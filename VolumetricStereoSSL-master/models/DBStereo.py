    ...
    def forward(self, x, y):
        ...
        with torch.cuda.device_of(x):
            ...
            for i in range(self.maxdisp):
                ...
                cost[:, x.size()[1]:, self.maxdisp-1-i, :, i:] = y[:,:,:,:width-i]
...
def Volume2Weight(block, maxdisp, width):
    through_1 = 1.0 - block
    cumprod = torch.cumprod(through_1, dim=2)
    w = block.clone()
    w[:, :, 1:, :, :] = cumprod[:, :, :maxdisp - 1, :, :] * block[:, :, 1:, :, :]
    return w

def ColorVolume(x_full, y_full, sizes):
    batch_size, channels, maxdisp, height, width = sizes
    color_1 = x_full.new().resize_(batch_size, 3, maxdisp, height, width).zero_()
    color_2 = x_full.new().resize_(batch_size, 3, maxdisp, height, width).zero_()
    for i in range(maxdisp):
        color_1[:, :, maxdisp - 1 - i, :, :width] = y_full[:, :, :, maxdisp-i:maxdisp-i+width]
        # color_2[:, :, maxdisp - 1 - i, :, :width-i] = x_full[:, :, :, i:width]
        color_2[:, :, maxdisp - 1 - i, :, :width] = x_full[:, :, :, i:i+width]   
...
class VolumeRender(nn.Module):
    ...
    def forward(self, v, x, y, x_full, y_full, mode=""):
        ...
        with torch.cuda.device_of(v):
            ...
            for i in range(self.maxdisp):
                block_2[:, :, self.maxdisp - 1 - i, :, :width-i] = block[:, :, self.maxdisp - 1 - i, :, i:width]

            w2 = Volume2Weight(block_2, self.maxdisp, width)
            ...
            clv_1, clv_2 = ColorVolume(x_full, y_full, sizes)

            color_1 = torch.sum(w * clv_1, 2)
            color_2 = torch.sum(w2 * clv_2, 2)
...
class Model(nn.Module):
    ...
    def forward(self, x, y, x_full=None, y_full=None, mode=""):
        num, channels, height, width = x.size()
        ...
        return self.disp(cv1, x, y, x_full, y_full, mode)
